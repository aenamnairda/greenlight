...
func ValidatePasswordPlaintext(v *validator.Validator, password string) {
	...
	v.Check(len(password) <= 72, "password", "most not be more than 72 bytes")
}

func ValidateUser(v *validator.Validator, user *User) {
	v.Check(user.Name != "", "user", "must be provided")
	v.Check(len(user.Name) <= 50, "user", "must not be more than 500 bytes")
...
func ValidateUserID(v *validator.Validator, userID int64) {
	v.Check(userID > 0, "user_id", "must be a positive integer")
}
